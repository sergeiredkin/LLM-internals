#!/usr/bin/env python3
"""Export a portable inference checkpoint with INT8 transformer linear weights."""
...
def main() -> None:
    ...
    if source.get("format_version") != 1:
        raise SystemExit("INT8 export expects a full format-version-1 checkpoint")
    ...
    print(f"INT8 modules:      {len(quantized_modules)}")
    print(f"SHA-256:           {checksum}")
    print("Excluded:          tied token embedding / LM head")
    print("Kernel:            dequantize then floating-point F.linear")
