...
def evaluate_answer(record: dict) -> dict[str, object]:
    ...
    sentence_support = [
        len(answer_tokens(sentence) & evidence_word_set) / max(len(answer_tokens(sentence)), 1)
        for sentence in sentences
    ]
    unsupported_sentences = sum(score < 0.35 for score in sentence_support)
    ...
    return {
        ...
        "sentence_support": min(sentence_support) if sentence_support else 1.0,
        "unsupported_sentence_rate": unsupported_sentences / max(len(sentences), 1),
        "numeric_grounding": (
        ...
        "grounded": bool(cited_ids <= evidence_ids and overlap >= 0.2 and
                          (min(sentence_support) if sentence_support else 1.0) >= 0.35 and
